...
myA=[
 [1.0 + N*eps, 1.0 -N*eps,   1.0 -N*eps,    1.0 -N*eps],
 [N*eps,       1.0 +N*eps, 1.0 -N*eps,    1.0 -N*eps],
 [N*eps,       N*eps,      1.0 + N*eps, 1.0 -N*eps],
 [N*eps,       N*eps,      N*eps,       1.0 +N*eps]
]
...
def Gauss(A, B):
    ...
    print("\n".join("X{0} =\t{1:10.20f}".format(i + 1, x) for i, x in
enumerate(X)))
    return X
